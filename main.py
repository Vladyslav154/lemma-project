...
async def upload_file(request: Request, file: UploadFile = File(...), authorization: Optional[str] = Header(None)):
    ...
    # Используем Redis для хранения ссылок на файлы
    r_kv = redis.from_url(os.getenv("REDIS_URL"), decode_responses=True)
